...
describe('receipt line items carry the line total, not the unit price', () => {
  ...
  test('the 6-pack is 9.60 and says so', () => {
    expect(r.lineItems[1]).toEqual({ description: 'Mini Chicken Pie — 6 × 1.60', unitAmount: 9.60, discountRate: 0, category: null, onBehalfOf: null });
    expect(r.lineItems.reduce((s, li) => s + li.unitAmount, 0)).toBeCloseTo(16.10, 2);
  });
  test('single units are untouched', () => {
    expect(r.lineItems[0]).toEqual({ description: 'Luncheon & Egg Bun', unitAmount: 2.50, discountRate: 0, category: null, onBehalfOf: null });
  });
