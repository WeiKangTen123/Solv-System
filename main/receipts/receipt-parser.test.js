...
describe('receipts/receipt-parser', () => {
  describe('normalise — where a bad model response is made harmless', () => {
    ...
    test('normalises time and prefixes category onto description if missing', () => {
      ...
      expect(res.description).toBe('[Meals] Business working lunch with client @ Dong Seoul Supply (12:01, Johor)');
    });
    ...
    test('normalises line items and their prices correctly', () => {
      ...
      expect(res.lineItems).toEqual([
        ...
        { description: 'Burger — 2 × 12.50', unitAmount: 25, discountRate: 0, category: null, onBehalfOf: null },
        { description: 'Fries', unitAmount: 4.5, discountRate: 10, category: null, onBehalfOf: null },
      ]);
    });
...
describe('receipt-parser — categories come from one list, and the reader describes rather than justifies', () => {
  ...
  test('a category the model reworded is canonicalised', () => {
    const r = parser.normalise({ ...good, category: 'air &  transport', description: 'Snacks' });
    expect(r.category).toBe('Air & Transport');
    expect(r.description).toBe('[Air & Transport] Snacks');
  });
