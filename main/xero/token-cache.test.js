...
describe('token-cache', () => {
  ...
  test('getAllTenants/getValidToken work for a token cached with the default (custom) type', async () => {
    const cache = tokenCache.forCompany('user-1');
  ...
  test('an expired oauth token refreshes via xero/oauth, not xero/connect', async () => {
    const cache = tokenCache.forCompany('user-2');
  ...
  test('the connection type survives a refresh — a second expiry still refreshes via the same mechanism', async () => {
    const cache = tokenCache.forCompany('user-3');
  ...
  test('getValidToken throws a clear error for a tenant with no cached token, even after attempting reconnect', async () => {
    const cache = tokenCache.forCompany('user-4');
  ...
  test('getValidToken transparently reconnects from a cold cache (e.g. right after a restart) instead of forcing a manual reconnect', async () => {
    ...
    reconnectXero.mockImplementation(async userId => {
      tokenCache.forCompany(userId).cacheToken('t6', 'Org', 'recovered-tok', new Date(Date.now() + 10 * 60_000), 'oauth');
    });
    const cache = tokenCache.forCompany('user-6');
  ...
  test('a reconnect failure surfaces the underlying reason, not just a generic "reconnect first"', async () => {
    ...
    const cache = tokenCache.forCompany('user-7');
  ...
  test('concurrent cold-cache calls for the same user share one reconnect, not one each (refresh-token rotation would break a second concurrent call)', async () => {
    ...
    reconnectXero.mockImplementation(userId => new Promise(resolve => {
      resolveReconnect = () => {
        tokenCache.forCompany(userId).cacheToken('t8a', 'Org', 'tok-a', new Date(Date.now() + 10 * 60_000));
        tokenCache.forCompany(userId).cacheToken('t8b', 'Org', 'tok-b', new Date(Date.now() + 10 * 60_000));
        resolve();
      };
    }));
    const cache = tokenCache.forCompany('user-8');
  ...
  test('concurrent calls on an expired token share ONE refresh (rotation would break the second)', async () => {
    const cache = tokenCache.forCompany('user-r');
  ...
  test('a refresh updates every tenant on the same connection, not only the one that asked', async () => {
    const cache = tokenCache.forCompany('user-m');
  ...
  test('pruneTenants drops orgs Xero no longer lists, in memory and on disk', async () => {
    ...
    expect(tokenCache.getPersistedTenants(u.companyId).map(t => t.tenantId)).toEqual(['t1', 't2']);
    cache.pruneTenants(['t1']);
    expect(cache.getAllTenants().map(t => t.tenant_id)).toEqual(['t1']);
    expect(tokenCache.getPersistedTenants(u.companyId).map(t => t.tenantId)).toEqual(['t1']);
  });

  test('clear() empties both tokens and tenants', () => {
    const cache = tokenCache.forCompany('user-5');
