...
async function refreshClientCredentialsToken(companyId) {
  ...
  if (!clientId || !clientSecret) {
    throw new Error('Xero credentials not configured — go to Settings to add your Client ID and Secret.');
  }
...
async function autoConnect(companyId) {
  ...
  for (const tenant of tenants) {
    ...
    logger.info('Xero org connected', { tenantName: tenant.tenantName, companyId });
  }
