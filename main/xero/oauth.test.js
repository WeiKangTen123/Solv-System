...
describe('xero/oauth', () => {
  ...
  beforeEach(() => {
    ...
    getCompanyConfig.mockReturnValue({ ...USER_CREDS });
  });
  ...
  describe('buildAuthorizeUrl', () => {
    ...
    test('throws a clear error when this specific user has no Web app credentials on file', () => {
      getCompanyConfig.mockReturnValue({}); // redirect URI is set (beforeEach), but this user's own client id/secret aren't
    ...
    test('builds a URL using THIS user\'s own client_id, not some other user\'s', () => {
      getCompanyConfig.mockReturnValue({ XERO_OAUTH_CLIENT_ID: 'this-users-client-id', XERO_OAUTH_CLIENT_SECRET: 'secret' });
      ...
      expect(getCompanyConfig).toHaveBeenCalledWith('user-1');
    });
  ...
  describe('refreshAuthCodeToken', () => {
    test('throws when there is no refresh token on file', async () => {
      getCompanyConfig.mockReturnValue({ ...USER_CREDS }); // has app creds, but never completed a connection
    ...
    test('persists the ROTATED refresh token Xero returns, not the old one', async () => {
      ...
      getCompanyConfig.mockReturnValue({ ...USER_CREDS, XERO_OAUTH_REFRESH_TOKEN: 'old-refresh-token' });
      ...
      expect(saveCompanyConfig).toHaveBeenCalledWith('user-1', { XERO_OAUTH_REFRESH_TOKEN: 'rt-BRAND-NEW' });
  ...
  describe('completeConnection', () => {
    test('exchanges the code, saves connection state, and caches every returned tenant', async () => {
      ...
      tokenCache.forCompany.mockReturnValue({ cacheToken, pruneTenants: jest.fn() });

      const tenants = await oauth.completeConnection('user-1', 'the-code');

      expect(saveCompanyConfig).toHaveBeenCalledWith('user-1', expect.objectContaining({
